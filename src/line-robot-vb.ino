...
*/
  ...
  //Define thresolds
  int threshold = 3500; //TBC - average of a sensor when it is on black track and off-black/white track
  int thresholdHigh = 3800;

 // moveForward(wheelSpeedForward);
  
  if ((s2 >= threshold && s2 < thresholdHigh)   && (s3 >= threshold && s3 < thresholdHigh)) {
    // All sensors detect black, move forward
    moveForward(wheelSpeedForward);
     Serial.println("----moveForward------");
  } 
  
   if (s2 < threshold  || s2 >= thresholdHigh) {
     turnRight(wheelSpeedForward, wheelSpeedSlow);
     Serial.println("----turnRight------");
  } 
  
   if (s3 < threshold || s3 >= thresholdHigh) {
...
void loop() {  
  ...
  if (state->isIdle()) {                 // IDLE is when the robot is not doing anything
    ...
    primedActions();
      ...
      // DRIVING, the robot is actively driving, so do whatever needs to be done to continue driving for this tick
    drivingActions();
  }
